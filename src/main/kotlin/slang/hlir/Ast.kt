...
data class ProgramUnit(val stmt: List<Stmt>) : SlastNode()
...
sealed class Expr : SlastNode() {
    ...
    data class InlinedFunction(val params: List<String>, val body: Stmt.BlockStmt) : Expr()
}
...
fun main() {
    ...
    val ast = IRBuilder.visit(parseTree) as ProgramUnit //    print(ast.collectFunctionDeclarations())
    println(ast.prettyPrint()) //    print(prettyPrintAST(ast))
}
