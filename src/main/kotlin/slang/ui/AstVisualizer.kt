...
fun parseProgram(
    input: String,
    errorListModel: DefaultListModel<String>,
    statusLabel: JLabel,
): SlastNode {
...
fun SlastNode.toTreeNode(): DefaultMutableTreeNode =
    when (this) {
        ...
        is Stmt.LetStmt ->
            DefaultMutableTreeNode("LetStmt(${this.prettyPrint()})").apply {
                add(DefaultMutableTreeNode("id=$name"))
                add(expr.toTreeNode())
            }

        is Stmt.AssignStmt ->
            DefaultMutableTreeNode("AssignStmt(${this.prettyPrint()})").apply {
                add(lhs.toTreeNode())
                add(expr.toTreeNode())
            }

        is Expr.InlinedFunction ->
            DefaultMutableTreeNode("InlineFunction").apply {
                params.forEach { add(DefaultMutableTreeNode(it)) }
                add(body.toTreeNode())
            }

        is Stmt.Function ->
            DefaultMutableTreeNode("FunImpure($name)").apply {
                params.forEach { add(DefaultMutableTreeNode(it)) }
                add(body.toTreeNode())
            }

        is Stmt.WhileStmt ->
            DefaultMutableTreeNode("While").apply {
                add(condition.toTreeNode())
                add(body.toTreeNode())
            }

        is Stmt.PrintStmt ->
            DefaultMutableTreeNode("PrintStmt").apply {
                args.forEach { add(it.toTreeNode()) }
            }

        is Stmt.IfStmt ->
            DefaultMutableTreeNode("IfStmt").apply {
                add(condition.toTreeNode())
        ...
        is Expr.BinaryExpr ->
            DefaultMutableTreeNode("BinaryExpr(${this.prettyPrint()})").apply {
                add(left.toTreeNode())
        ...
        is Expr.IfExpr ->
            DefaultMutableTreeNode("IfExpr").apply {
                add(condition.toTreeNode())
        ...
        is Expr.Record ->
            DefaultMutableTreeNode("Record").apply {
                expression.forEach {
                    add(DefaultMutableTreeNode("ID(${it.first})"))
                    add(
                        DefaultMutableTreeNode("Expr(${it.second.prettyPrint()})").apply {
        ...
        is Stmt.DerefStmt ->
            DefaultMutableTreeNode("DerefStmt(${this.prettyPrint()})").apply {
                add(lhs.toTreeNode())
                add(rhs.toTreeNode())
            }

        is Expr.FieldAccess ->
            DefaultMutableTreeNode("FieldAccess(${this.prettyPrint()})").apply {
                add(lhs.toTreeNode())
                add(DefaultMutableTreeNode(rhs))
            }

        is Expr.ArrayAccess ->
            DefaultMutableTreeNode("ArrayAccess(${this.prettyPrint()})").apply {
                add(array.toTreeNode())
                add(index.toTreeNode())
            }
        is Expr.ArrayInit ->
            DefaultMutableTreeNode("ArrayInit").apply {
                elements.forEach { add(it.toTreeNode()) }
            }
        Stmt.Break -> DefaultMutableTreeNode("break")
        Stmt.Continue -> DefaultMutableTreeNode("continue")
        is Stmt.StructStmt ->
            DefaultMutableTreeNode("StructStmt").apply {
                add(DefaultMutableTreeNode("id=$id"))
                fields.forEach {
                    add(
                        DefaultMutableTreeNode("Field(${it.component1()})").apply {
...
class ASTViewer : JFrame("Slang AST Visualizer") {
    ...
    private val inputScrollPane =
        RTextScrollPane(inputArea).apply {
    ...
    private fun updateCaretPosition() {
        ...
        val column: Int =
            caretPos -
                inputArea.document.defaultRootElement
                    .getElement(line)
                    .startOffset
